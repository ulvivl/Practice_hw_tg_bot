...
class FriendsBirths:
    ...
    def _births(self):
        ...
        for friend in vk_friends['items']:
            if  not (not friend.get('deactivated') is None and  friend.get('deactivated') == 'deleted'):
                date_of_birth = friend.get('bdate')
                if not date_of_birth is None:
                    fir_name, sec_name = friend.get('first_name'), friend.get('last_name')
                    day, mon = date_of_birth.split('.')[0], date_of_birth.split('.')[1]
                    date_b = str(datetime.now().year) + ' ' + str(mon) + ' ' + str(day)
                    d1 = date(int(y), int(mon), int(day))
                    d2 = date(int(y), int(m), int(d))
                    diff = (d1 - d2).days
                    if diff < 0:
                        if ((int(y) + 1) % 4 == 0 and (int(y) + 1) % 100 != 0) or ((int(y) + 1) % 400 == 0):
                            diff += 366
                        else:
                            diff += 365
                    if diff <= self._count:
                        fir_name, sec_name = friend.get('first_name'), friend.get('last_name')
                        full_name = ''
                        if not fir_name is None:
                            full_name += fir_name
                        if not sec_name is None:
                            full_name += ' '
                            full_name += sec_name
                        near_births[full_name] = [diff, day, months[int(mon) - 1], str(friend['id'])]
...
def send_welcome(message):
    ...
    global command
    if command == 1:
        bot.send_message(chat_id, "Сначала закончите выполнение предыдущей команды")
    else:
        bot.send_message(chat_id, f'Здравствуйте, {message.from_user.first_name}\nДля ознакомления с ботом напишите команду /help')
...
def change_vk_id(message):
    ...
    if command == 1:
        ...
        changed_id = 1
        command = 1
        bot.send_message(chat_id, 'Введите новый vk id в формате 173349671\nПо данной ссылке вы можете найти vk id: https://regvk.com/id/')
...
def get_text_messages(message):
    ...
    if changed_id == 1:
        if not mess.isdigit():
            was_id = 0
            bot.reply_to(message, 'Неправильный формат vk id, попробуйте снова')
        else:
          was_id = 1
          id = int(mess)
    elif changed_days == 1:
        ...
        if was_id == 0:
            if not mess.isdigit():
                ...
                id = int(mess)
                was_id = 1
                bot.send_message(chat_id, 'Введите количество дней насколько дней вперед вы хотите узнать дни рождения')
        else:
            if not mess.isdigit():
                bot.reply_to(message, 'Неправильный формат количества дней, попробуйте снова')
            else:
                kol = int(mess)
                was_count = 1
    if was_id == 1 and was_count == 1:
          ...
          if (sort_births == -1):
              bot.send_message(chat_id, 'Данный профиль закрыт или не существует.\nДля использования бота удостоверьтесь, что профиль существует или сделайте профиль не приватным и вызовите команду /change_id')
          else:
              changed_id = 0
              if (len(sort_births) == 0):
                    bot.send_message(chat_id, "В ближайшее указанное количество дней нет дней рождения")
              else:
                  days_count = ''
                  for person in sort_births:
                      if (person[1][0] == 0):
                          bot.send_message(chat_id, "У пользователя: " + person[0] + '\n' + 'https://vk.com/id' + str(person[1][3]) + '\n' + 'Cегодня день рождения' + ' ' + '(' + person[1][1] + ' ' + person[1][2] + ')')
                          continue
                      if (str(person[1][0])[-1] == '1' and person[1][0] != 11):
                          ...
                          days_count = 'дней'
                      bot.send_message(chat_id, "У пользователя: " + person[0] +  '\n' + 'https://vk.com/id' + str(person[1][3]) +  '\n' + 'День рождения через ' + str(person[1][0]) + ' ' + days_count + ' ' + '(' + person[1][1] + ' ' + person[1][2] + ')')
                      time.sleep(0.05)
          command = 0
